...
const shouldRemoveFeature = (featureDirName) => {
  return keepMode
    ? !keepFeatures.includes(featureDirName)
    : removeFeatures.includes(featureDirName);
};
...
const performFeatureRegionRemoval = async () => {
  ...
  await Promise.all(
    featureRegionReferences.map(async (reference) => {
      try {
        ...
        for (const feature of reference.features) {
          ...
          while (startIndex >= 0 && endIndex >= 0) {
            ...
            newData =
              ...
              newData.substring(endIndex + endString.length + 1);
...
const performRemovals = async () => {
  ...
  if (shouldRemoveFeature('chat-to-video-escalation')) {
    shell.echo(`Deleting addons/twilio-video-demo-app...`);
  ...
  if (keepMode && keepFeatures.length === 0) {
    shell.echo(`Deleting managed terraform assets for studio and taskrouter`);
